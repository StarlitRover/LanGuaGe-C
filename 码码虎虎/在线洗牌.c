...
and in order to avoid "inside jobs" where employees collaborate with gamblers by performing inadequate shuffles, many casinos 
...
Input Specification:
Each input file contains one test case. For each case, the first line contains a positive integer K (¡Ü20) which is the number of repeat times. 
...
int main() 
{
    int n, i, c[108], change[54], flag0, flag1;
    char l;
